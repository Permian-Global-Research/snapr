#' Random-Forest-Classifier: snap operator function
#' @param operator_id character operator id
#' @param sourceProduct The source product(s) as input to the operator
#' @param treeCount The number of trees Valid interval is (1,*\]. Default value
#' is '10'.
#' @param numTrainSamples The number of training samples Valid interval is
#' (1,*\]. Default value is '50000'.
...
#' value is '101'.
#' @param trainOnRaster Train on raster or vector data Default value is
#' 'false'.
...
#' @param evaluateClassifier Evaluate classifier and features Default value is
#' 'false'.
...
#'
#' "Random Forest based classifier Parameter Options: -PclassLevels=<int>
#' Quantization class levels for raster traiing Default value is '101'.
...
#' Evaluate classifier and features Default value is 'false'.
#' -PevaluateFeaturePowerSet=<boolean> Evaluate the power set of features Default
#' value is 'false'. -PfeatureBands=<string,string,string,...> Names of bands to
#' be used as features -PlabelSource=<string> 'VectorNodeName' or specific
#' Attribute name -PmaxPowerSetSize=<integer> Maximum size of the power set of
#' features Default value is '7'. -PminClassValue=<double> Quantization min class
#' value for raster traiing Default value is '0.0'. -PminPowerSetSize=<integer>
#' Minimum size of the power set of features Default value is '2'.
#' -PnumTrainSamples=<int> The number of training samples Valid interval is (1,*\].
#' Default value is '50000'. -PsavedClassifierName=<string> The saved classifier
...
#' 'false'. -PtreeCount=<int> The number of trees Valid interval is (1,*\]. Default
#' value is '10'."
#' @import xml2
#' @return snap_op_random_forest_classifier object
#' @export
op_random_forest_classifier <- function(
    operator_id,
    sourceProduct,
    treeCount = 10,
  ...
  xml_add_child(node, "operator", "Random-Forest-Classifier")
  ...
  snap_op_random_forest_classifier <- S7::new_class(
    "snap_op_random_forest_classifier",
    parent = snap_operator
  )
  snap_op_random_forest_classifier(
    operator = "Random-Forest-Classifier",
