#' Maximum-Likelihood-Classifier: snap operator function
...
#' 'false'.
#' @param doClassValQuantization Quantization for raster training Default value
#' is 'true'.
#' @param minClassValue Quantization min class value for raster training
#' Default value is '0.0'.
#' @param classValStepSize Quantization step size for raster training Default
#' value is '5.0'.
#' @param classLevels Quantization class levels for raster training Default
#' value is '101'.
...
#'
#' "Maximum Likelihood classifier Parameter Options: -PclassLevels=<int>
#' Quantization class levels for raster training Default value is '101'.
#' -PclassValStepSize=<double> Quantization step size for raster training Default
#' value is '5.0'. -PdoClassValQuantization=<boolean> Quantization for raster
#' training Default value is 'true'. -PdoLoadClassifier=<boolean> Choose to save
#' or load classifier Default value is 'false'. -PevaluateClassifier=<boolean>
...
#' training Default value is '0.0'. -PminPowerSetSize=<integer> Minimum size of
#' the power set of features Default value is '2'. -PnumTrainSamples=<int> The
#' number of training samples Valid interval is (1,*\]. Default value is '5000'.
#' -PsavedClassifierName=<string> The saved classifier name
#' -PtrainingBands=<string,string,string,...> Raster bands to train on
...
#' 'true'."
#' @import xml2
#' @return snap_op_maximum_likelihood_classifier object
#' @export
op_maximum_likelihood_classifier <- function(
    operator_id,
    sourceProduct,
  ...
  xml_add_child(node, "operator", "Maximum-Likelihood-Classifier")
  ...
  snap_op_maximum_likelihood_classifier <- S7::new_class(
    "snap_op_maximum_likelihood_classifier",
    parent = snap_operator
  )
  snap_op_maximum_likelihood_classifier(
    operator = "Maximum-Likelihood-Classifier",
