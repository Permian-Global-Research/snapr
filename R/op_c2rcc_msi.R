#' c2rcc.msi: snap operator function
...
#' processing. Default value is 'B8 > 0 && B8 < 0.1'.
...
#' @param elevation Used as fallback if elevation could not be taken from
#' GETASSE30 DEM. Valid interval is (0, 8500). Default value is '0'. Parameter
#' unit is 'm'.
...
#' flag for gas corrected top-of-atmosphere reflectances Default value is '0.05'.
#' @param thresholdAcReflecOos Threshold for out of scope of nn training
#' dataset flag for atmospherically corrected reflectances Default value is '0.1'.
...
#' this to replace the standard set of neuronal nets.
#' @param netSet Set of neuronal nets for algorithm. Value must be one of
#' 'C2RCC-Nets', 'C2X-Nets', 'C2X-COMPLEX-Nets'. Default value is 'C2RCC-Nets'.
...
#' @param useEcmwfAuxData Use ECMWF auxiliary data (msl and tco3) from the
#' source product, if available. Default value is 'false'.
...
#' Sentinel-2 MSI L1C data products."
#' @import xml2
#' @return snap_op_c2rcc_msi object
#' @export
op_c2rcc_msi <- function(
    operator_id,
    sourceProduct,
    ncepEndProduct = NULL,
    ...
    validPixelExpression = "B8 > 0 && B8 < 0.1",
    salinity = 35.0,
    temperature = 15.0,
    ozone = 330,
    press = 1000,
  ...
  xml_add_child(node, "operator", "c2rcc.msi")
  ...
  snap_op_c2rcc_msi <- S7::new_class(
    "snap_op_c2rcc_msi",
